...
const submit = (id) => {
    router.put(`/tickets/${id}`, form);
};

onMounted(() => {
    form.driver_id = props.ticket.driver_id;
    form.enforcer_id = props.ticket.enforcer_id;
    form.ticket_no = props.ticket.ticket_no;
    form.violations = props.ticket.violations.map((v) => v.id);
    form.status = props.ticket.status;
    form.location_of_incident = props.ticket.location_of_incident;
    form.date_of_incident = props.ticket.date_of_incident;
});
</script>
...
<template>
    ...
    <FormLayout>
        <PageHeading pageTitle="Create Ticket" link="/tickets" />

        <form @submit.prevent="submit(ticket.id)" class="mt-5 space-y-10">
